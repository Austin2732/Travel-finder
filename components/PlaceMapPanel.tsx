...
export default function PlaceMapPanel({
  destination,
  options,
  selectedId,
  onSelect,
  ...
  return (
    <aside className="sticky top-24 overflow-hidden rounded-3xl border border-surface-200 bg-white shadow-card">
      <div className="border-b border-surface-100 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600">Live map</p>
        <h3 className="mt-1 font-display text-xl font-bold text-surface-900">
          {destination ? `${destination.name} pins` : "Trip pins"}
        </h3>
        <p className="mt-1 text-sm text-surface-500">
          Tap a pin below to preview it on the map.
        </p>
      </div>

      <div className="relative h-72 bg-surface-100">
        <iframe
          key={embedUrl}
          title={`${mapTitle} map`}
          src={embedUrl}
        ...
        <a
          ...
          className="absolute bottom-3 right-3 inline-flex items-center gap-2 rounded-full bg-white/95 px-3 py-2 text-xs font-semibold text-surface-800 shadow-card hover:bg-white"
        >
      ...
      <div className="max-h-[420px] space-y-2 overflow-y-auto p-3">
        {mappableOptions.length === 0 && (
          <div className="rounded-2xl bg-surface-50 p-4 text-sm text-surface-500">
            Map pins will appear here when live Google Places results include coordinates.
          </div>
        )}
            ...
            >
              <span
                className={cn(
                  ...
                  isActive ? "bg-brand-600 text-white" : "bg-surface-900 text-white"
                )}
              >
                {index + 1}
              </span>
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold text-surface-900">{option.title}</span>
                <span className="mt-0.5 flex items-center gap-1 text-xs text-surface-500">
                  <MapPin className="h-3 w-3" />
                  <span className="truncate">{option.address.city}{option.address.region ? `, ${option.address.region}` : ""}</span>
                </span>
                {rating && rating.score > 0 && (
                  <span className="mt-1 block text-xs font-medium text-amber-600">
                    {(rating.score / 2).toFixed(1)}★ · {rating.reviewCount.toLocaleString()} reviews
                  </span>
                )}
              </span>
...
function buildGoogleMapsSearchUrl(query: string, geo?: { lat: number; lng: number }) {
  const encodedQuery = encodeURIComponent(query);
  if (geo) {
    return `https://www.google.com/maps/search/?api=1&query=${geo.lat},${geo.lng}&query_place_id=${encodedQuery}`;
  }
